(function () {
  ...
  var VERSION = 'v1.9.0-r2-delete-hardening';
  var state = {
    stores: [],
    contents: [],
    devices: [],
  ...
  if (originalFetch && !window.__LV_RIGHT_TARGETS_FETCH_PATCHED_V190__) {
    window.__LV_RIGHT_TARGETS_FETCH_PATCHED_V190__ = true;
  ...
  function installCss() {
    if (document.getElementById('lv-right-target-style-v190')) return;
    var style = document.createElement('style');
    style.id = 'lv-right-target-style-v190';
    style.textContent = `
  ...
  function ensureBadge() {
    ...
    badge.textContent = 'Right Target UI v1.9.0 로드됨';
  ...
  function bindSideChange() {
    var card = findContentCard();
    var select = findSideSelect(card);
    if (select && !select.dataset.lvTargetBoundV190) {
      select.dataset.lvTargetBoundV190 = '1';
