(function () {
  ...
  var VERSION = 'v1.8.8-right-target-visibility';
  var state = {
    stores: [],
    contents: [],
    devices: [],
  ...
  if (originalFetch && !window.__LV_RIGHT_TARGETS_FETCH_PATCHED_V188__) {
    window.__LV_RIGHT_TARGETS_FETCH_PATCHED_V188__ = true;
  ...
  function isContentPageLike() {
    ...
    return bodyText.indexOf('우측 30% 공통 콘텐츠') >= 0 || bodyText.indexOf('콘텐츠 관리') >= 0 || Boolean(findContentCard());
  }
  ...
  function findCandidateCardForContent(content) {
    ...
    var all = Array.from(document.querySelectorAll('article, section, .content-card, .panel, .card, div'));
    var matches = [];
    for (var i = 0; i < all.length; i++) {
      var el = all[i];
      if (!el || !el.querySelector || el.id === 'root' || el.closest('#lv-right-target-panel') || el.closest('#lv-right-visibility-modal')) continue;
      var txt = textOf(el);
      ...
      var rightHint = txt.indexOf('stores/_common/right') >= 0 || txt.indexOf('공통 우측') >= 0 || txt.indexOf('우측 30') >= 0 || txt.indexOf('미디어 열기') >= 0;
      if ((titleHit || fileHit || keyHit) && (rightHint || fileHit || keyHit)) {
        try {
  ...
  function makeVisibilityControls(content) {
    ...
    wrap.setAttribute('data-lv-visibility-for', content.id || content.title || content.fileName || 'right');
    var target = getContentTarget(content);
    ...
    btn.addEventListener('click', function (ev) {
      ev.preventDefault();
      ev.stopPropagation();
      if (!state.stores.length || !state.devicesLoaded || !state.contentsLoaded) {
        btn.disabled = true;
        var oldText = btn.textContent;
        btn.textContent = '목록 불러오는 중...';
        Promise.resolve(loadVisibilityData(false)).finally(function () {
          btn.disabled = false;
          btn.textContent = oldText;
          openVisibilityModal(content);
        });
      } else {
        openVisibilityModal(content);
      }
  ...
  function openVisibilityModal(content) {
    ...
    box.querySelector('.lv-modal-close').addEventListener('click', close);
  }
  ...
  function installCss() {
    if (document.getElementById('lv-right-target-style-v188')) return;
    var style = document.createElement('style');
    style.id = 'lv-right-target-style-v188';
    style.textContent = `
  ...
  function ensureBadge() {
    ...
    badge.textContent = 'Right Target UI v1.8.8 로드됨';
  ...
  function bindSideChange() {
    var card = findContentCard();
    var select = findSideSelect(card);
    if (select && !select.dataset.lvTargetBoundV188) {
      select.dataset.lvTargetBoundV188 = '1';
