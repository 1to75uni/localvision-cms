(function () {
  ...
  if (originalFetch && !window.__LV_RIGHT_TARGETS_FETCH_PATCHED_V189__) {
    window.__LV_RIGHT_TARGETS_FETCH_PATCHED_V189__ = true;
  ...
  function installCss() {
    if (document.getElementById('lv-right-target-style-v189')) return;
    var style = document.createElement('style');
    style.id = 'lv-right-target-style-v189';
    style.textContent = `
  ...
  function bindSideChange() {
    var card = findContentCard();
    var select = findSideSelect(card);
    if (select && !select.dataset.lvTargetBoundV189) {
      select.dataset.lvTargetBoundV189 = '1';
